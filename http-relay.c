...
static void httpr_client_init(redsocks_client *client)
{
	...
	if (client->instance->config.login)
		log_error("login is ignored for http-relay connections");
	
	if (client->instance->config.password)
		log_error("password is ignored for http-relay connections");
...
static int httpr_toss_http_firstline(redsocks_client *client)
{
	...
	if (uri) 
		...
		log_error("malformed request came");
		goto fail;
	}

	buff = evbuffer_new();
	if (!buff) {
		log_error("evbuffer_new");
		goto fail;
	}
